...
export default function EventItemAddForm({ eventId, eventTitle, categories, brands, onAdd }) {
  const [form, setForm] = useState({ name: '', category: '', brand: '', add_quantity: '', price: '' });
  const [submitting, setSubmitting] = useState(false);

  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.name || !form.category || !form.brand || form.add_quantity === '' || form.price === '' || form.price === null || form.price === undefined) return;
    setSubmitting(true);
    try {
      await onAdd(form);
      setForm({ name: '', category: '', brand: '', add_quantity: '', price: '' });
    ...
    <div className="bg-white rounded-xl border border-border p-5">
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        ...
        <div>
          ...
          <select value={form.brand} onChange={e => setForm({ ...form, brand: e.target.value })} className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm" required>
