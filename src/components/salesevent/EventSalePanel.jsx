  ...
  if (visibleItems.length === 0) {
    return (
      <p className="text-sm text-muted-foreground text-center py-4">
        {items.length > 0
          ? 'هیچ آیتمی برای نمایش فعال نیست. از «ویرایش آیتم‌های ایونت» گزینه «نمایش» را فعال کنید.'
          : 'ابتدا از «ویرایش آیتم‌های ایونت» آیتمی برای این ایونت اضافه کنید'}
      </p>
    );
  }
            ...
            {catItems.map(item => {
              ...
              return (
                <button
                  ...
                  disabled={disabled}
                  className={`flex items-center justify-between px-3 py-2 rounded-lg border text-sm transition-colors ${disabled ? 'opacity-40 cursor-not-allowed border-border' : inCart ? 'border-[#B74B40] bg-[#FDF2F1]' : 'border-border hover:bg-muted/30'}`}
                >
        ...
        <div className="border-t border-border pt-4">
          ...
          <div className="mt-3 pt-3 border-t border-border flex items-center gap-3 flex-wrap">
            ...
            <input
              ...
              placeholder="0"
              value={cartDiscount || ''}
              onChange={e => { const raw = persianToEnglish(e.target.value).replace(/[^0-9]/g, ''); setCartDiscount(raw ? Number(raw) : 0); }}
              className="w-24 px-2 py-1 rounded-lg border border-input bg-background text-xs text-center"
              dir="ltr"
            />
