  ...
  return (
    <div className="bg-white rounded-xl border border-border overflow-hidden">
      <div className="p-4 border-b border-border">
        <h3 className="text-sm font-semibold">فهرست آیتم‌های ایونت {eventTitle} ({toPersianNum(items.length)})</h3>
      </div>
      {items.length === 0 ? (
                  ...
                  <tr className="border-t border-border hover:bg-muted/30">
                    ...
                    <td className="p-3 text-center">
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => (editingId === item.id ? cancelEdit() : startEdit(item))} className="text-muted-foreground hover:text-[#B74B40]"><Pencil className="w-4 h-4" /></button>
                        <button onClick={() => setDeleteTarget(item.id)} className="text-muted-foreground hover:text-red-600"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    </td>
                  </tr>
                  {editingId === item.id && (
