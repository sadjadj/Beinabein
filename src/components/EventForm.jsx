    ...
    <div className="bg-white rounded-xl border border-border p-5">
      <form onSubmit={handleSubmit} className="space-y-3">
        <div className="flex flex-wrap items-end gap-3">
          <div>
            <label className="text-xs text-muted-foreground block mb-1">اسم رخداد</label>
            <input type="text" placeholder="اسم رخداد" value={form.title || ''} onChange={e => setForm({ ...form, title: e.target.value })} className="px-3 py-2 rounded-lg border border-input bg-background text-sm w-48" required />
          </div>
          <div>
        ...
        <div className="flex flex-wrap items-end gap-3">
          ...
          <div>
            <label className="text-xs text-muted-foreground block mb-1">روز رخداد (خودکار)</label>
            <div className="px-3 py-2 rounded-lg border border-input bg-muted/50 text-sm whitespace-nowrap w-fit min-w-[140px]">
              {form.day_of_week || '—'}
            </div>
          </div>
        </div>
        <textarea placeholder="توضیحات رخداد" value={form.description || ''} onChange={e => setForm({ ...form, description: e.target.value })} rows={2} className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm" />
        <div>
        ...
        <div className="flex justify-between gap-2">
          {showDelete && onDelete && (
            <button type="button" onClick={() => setShowDeleteConfirm(true)} className="flex items-center gap-1 px-4 py-2 rounded-lg border border-red-200 text-red-600 text-sm font-medium hover:bg-red-50">
              <Trash2 className="w-4 h-4" /> حذف رخداد
            </button>
          )}
        ...
        <AlertDialogContent className="text-center">
          <AlertDialogHeader className="text-center">
            ...
            <AlertDialogDescription className="text-center block">
              آیا از حذف این رخداد اطمینان دارید؟ این عملیات قابل بازگشت نیست.
