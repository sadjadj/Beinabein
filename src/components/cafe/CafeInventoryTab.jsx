...
export default function CafeInventoryTab({ items, categories, onItemSubmit, onDeleteItem, onToggleVisible, onEditCategories }) {
  ...
  const handleAddSubmit = async (e) => {
    e.preventDefault();
    if (!itemForm.name || itemForm.price === '' || itemForm.price === null || itemForm.price === undefined) return;
    setSubmitting(true);
    try {
      await onItemSubmit(itemForm, null);
  ...
  const saveEdit = async (e) => {
    ...
    setSubmitting(true);
    try {
      await onItemSubmit(editForm, editingItemId);
      cancelEdit();
    } finally { setSubmitting(false); }
  };
  ...
  return (
    <div className="space-y-4">
      <div className="bg-white rounded-xl border border-border p-5">
        <h3 className="text-sm font-semibold mb-4 flex items-center gap-2"><Plus className="w-4 h-4 text-[#B74B40]" /> افزودن آیتم جدید</h3>
        <form onSubmit={handleAddSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          <div>
            ...
            <input type="text" placeholder="اسم آیتم" value={itemForm.name} onChange={e => setItemForm({ ...itemForm, name: e.target.value })} className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm" required />
          </div>
          <div>
            ...
            <select value={itemForm.category} onChange={e => setItemForm({ ...itemForm, category: e.target.value })} className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm">
                    ...
                    <tr className="border-t border-border hover:bg-muted/30">
                      ...
                      <td className="p-3 text-center">
                        <input
                          ...
                          onChange={() => onToggleVisible(item.id, item.is_visible !== false)}
                      ...
                      <tr className="border-t border-border bg-muted/20">
                        <td colSpan={6} className="p-4">
                          <form onSubmit={saveEdit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 items-end">
                            ...
                            <div>
                              <label className="text-xs text-muted-foreground block mb-1">قیمت</label>
