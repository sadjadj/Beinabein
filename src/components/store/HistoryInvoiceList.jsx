...
import { ChevronDown, ChevronUp, Check, X, Pencil, Trash2 } from 'lucide-react';
...
export default function HistoryInvoiceList({ groups, onTogglePaid, onSaveEdit, onDelete, emptyMessage = 'در این بازه فاکتوری ثبت نشده است' }) {
  ...
  const startEdit = (g) => {
    setEditingKey(g.key);
    setEditForm({ payment_method: g.payment_method, purchase_reason: g.purchase_reason, is_paid: g.is_paid });
  };
  const saveEdit = (g) => { onSaveEdit(g, editForm); setEditingKey(null); };
          ...
          return (
            <div key={g.key} className="p-3">
              <div
                ...
                onClick={() => !isEditing && navigate(`/accounting/${g.invoiceType}/${g.items[0].id}`, { state: { from: location.pathname } })}
              >
                ...
                <div className="flex items-center gap-2">
                  {isEditing ? (
                    <>
                      ...
                      <button onClick={(e) => { e.stopPropagation(); saveEdit(g); }} className="text-green-600 hover:text-green-700"><Check className="w-4 h-4" /></button>
                      <button onClick={(e) => { e.stopPropagation(); setEditingKey(null); }} className="text-muted-foreground hover:text-foreground"><X className="w-4 h-4" /></button>
                    </>
                  ) : (
                    <>
                      <span className="text-xs text-muted-foreground">{paymentMethodLabels[g.payment_method] || g.payment_method}</span>
                      <button onClick={(e) => { e.stopPropagation(); onTogglePaid(g); }} className={`text-xs ${g.is_paid ? 'text-green-600' : 'text-[#B9834B]'}`}>{g.is_paid ? 'پرداخت شده' : 'پرداخت‌نشده'}</button>
                      <span className="font-medium text-sm">{formatCurrency(g.totalAmount)}</span>
                      <button onClick={(e) => { e.stopPropagation(); startEdit(g); }} className="text-muted-foreground hover:text-[#B74B40]"><Pencil className="w-3.5 h-3.5" /></button>
                      <button onClick={(e) => { e.stopPropagation(); setDeleteTarget(g); }} className="text-muted-foreground hover:text-red-600"><Trash2 className="w-3.5 h-3.5" /></button>
                    </>
                  )}
                </div>
              </div>
