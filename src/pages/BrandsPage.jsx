...
import { Tag, Plus, Search, Store } from 'lucide-react';
  ...
  return (
    <div className="p-4 md:p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 lg:gap-4">
        ...
        <StatCard label="با استودیو" value={toPersianNum(records.filter(r => r.brand_name).length)} icon={Store} color="ochre" />
        <StatCard label="فیلترشده" value={toPersianNum(filtered.length)} icon={Tag} color="teal" />
      </div>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-border p-5">
          <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            ...
            <input type="text" placeholder="نوع همکاری" value={form.collaboration_type} onChange={e => setForm({ ...form, collaboration_type: e.target.value })} className="px-3 py-2 rounded-lg border border-input bg-background text-sm" />
            ...
            <input type="text" placeholder="اسم استودیو/برند" value={form.brand_name} onChange={e => setForm({ ...form, brand_name: e.target.value })} className="px-3 py-2 rounded-lg border border-input bg-background text-sm" />
      ...
      <div className="bg-white rounded-xl border border-border overflow-hidden">
        <div className="p-4 border-b border-border flex items-center justify-between gap-3">
          <h3 className="text-sm font-semibold">فهرست برندها ({toPersianNum(filtered.length)})</h3>
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/50">
                <tr>
                  ...
                  <th className="text-right p-3 font-medium">استودیو/برند</th>
                  <th className="text-right p-3 font-medium">نوع همکاری</th>
                  <th className="text-right p-3 font-medium">نوع محصول</th>
                  <th className="text-right p-3 font-medium">آیدی</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map(r => (
                  <tr key={r.id} className="border-t border-border hover:bg-muted/30">
                    ...
                    <td className="p-3">{r.collaboration_type || '-'}</td>
                    <td className="p-3">{r.product_type || '-'}</td>
                    <td className="p-3 text-muted-foreground text-xs">{r.social_id || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
