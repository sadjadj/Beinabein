...
export default function ArtistsPage() {
  ...
  const filtered = records.filter(r => {
    if (brandFilter === 'with' && !r.brand_name) return false;
    if (brandFilter === 'without' && r.brand_name) return false;
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex items-center gap-1">
            {[['', 'همه'], ['with', 'دارای برند'], ['without', 'بدون برند']].map(([val, lbl]) => (
              <button key={val} onClick={() => setBrandFilter(val)} className={`px-2.5 py-1.5 rounded-full text-xs font-medium transition-colors ${brandFilter === val ? 'bg-[#B74B40] text-white' : 'bg-muted text-muted-foreground hover:bg-muted/80'}`}>{lbl}</button>
            ))}
          </div>
