              ...
              {showAddReg && (
                <div className="mb-3 p-3 bg-muted/30 rounded-lg grid grid-cols-1 sm:grid-cols-2 gap-2">
                  ...
                  <div>
                    ...
                    <select required value={regForm.plan_id} onChange={(e) => { setRegError(''); const plan = plans.find(p => p.id === e.target.value); const isFree = plan && (Number(plan.price) === 0 || plan.name === 'رایگان'); setRegForm(prev => ({ ...prev, plan_id: e.target.value, price: plan ? plan.price : (workshop.price || ''), is_paid: !!isFree, payment_method: isFree ? 'free' : 'cash' })); }} className="w-full px-3 py-2 rounded-lg border border-input bg-background text-sm">
